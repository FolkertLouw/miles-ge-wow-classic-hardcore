...
local function currentOrRecentSkinTarget()
  ...
  if target and target.name then
    return { mob = target, source = "current_target", confidence = "medium" }
  end
  local corpse = closestRecentCorpse()
  if corpse then
    return { mob = corpse.mob, source = "recent_defeat", corpse = corpse, confidence = "medium" }
  end
  return { mob = nil, source = "unknown", confidence = "low" }
end
...
frame:SetScript("OnEvent", function(self, event, ...)
  ...
  if event == "UNIT_SPELLCAST_SUCCEEDED" then
    ...
    logSkinningEvent("skinning_succeeded", {
      ...
      spellID = spellID,
      spellName = spellName,
      targetContext = lastSkinning and lastSkinning.targetContext or currentOrRecentSkinTarget(),
      confidence = lastSkinning and lastSkinning.targetContext and lastSkinning.targetContext.confidence or "medium",
    })
    return
  end
